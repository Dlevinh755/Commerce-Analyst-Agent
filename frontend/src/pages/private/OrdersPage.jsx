...
export default function OrdersPage() {
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border-2 border-stone-200 bg-white p-5 shadow-sm">
        <div>
          <h1 className="text-2xl font-extrabold text-ink">Đơn hàng người bán</h1>
          <p className="mt-1 font-semibold text-stone-600">Đánh dấu đã gửi khi đơn hàng rời kho của bạn.</p>
        </div>
        <button
          type="button"
          className="rounded-lg border-2 border-stone-300 px-3 py-2 text-sm font-bold text-ink hover:bg-brand-50"
          onClick={loadOrders}
        >
          Làm mới
        </button>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="flex items-center gap-3 rounded-xl border-2 border-stone-200 bg-white p-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-stone-600">Đơn chờ xử lý</p>
            <p className="text-lg font-extrabold text-ink">{summaryCounts.pending}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-xl border-2 border-stone-200 bg-white p-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-stone-600">Đơn đã gửi</p>
            <p className="text-lg font-extrabold text-ink">{summaryCounts.shipped}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 rounded-xl border-2 border-stone-200 bg-white p-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-stone-600">Đơn đã giao</p>
            <p className="text-lg font-extrabold text-ink">{summaryCounts.delivered}</p>
          </div>
        </div>
      </section>

      <div className="rounded-2xl border-2 border-stone-200 bg-white p-4 shadow-sm md:p-5">
        <div className="flex flex-wrap items-center gap-2 border-b-2 border-stone-200 pb-4">
        ...
        {loading ? (
          ...
          <div className="mt-3 space-y-3">
            {filteredOrders.map((order) => {
              ...
              return (
                <article key={order.order_id} className="rounded-2xl border-2 border-stone-200 bg-white p-4 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <label className="flex items-start gap-3">
                      <input type="checkbox" className="mt-1 h-4 w-4 rounded border-slate-300" />
                      <div>
                        <p className="text-sm font-extrabold text-ink">#{order.order_id}</p>
                        <p className="text-xs font-semibold text-stone-600">{order.buyerLabel}</p>
                      </div>
                    </label>

                    <div className="text-right text-xs text-slate-500">
                      <p className="font-medium text-slate-700">Ngày đặt</p>
                  ...
                  <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_minmax(240px,0.8fr)]">
                    <div className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3">
                      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Sản phẩm cần gửi</p>
                      {sellerItems.length ? (
                        <div className="space-y-2">
                          {sellerItems.map((item) => {
                            ...
                            return (
                              <div key={item.order_item_id || item.id || `${order.order_id}-${item.book_id}`} className="flex min-w-0 gap-2 rounded-lg border border-slate-200 bg-white p-2">
                                <div className="h-12 w-9 flex-shrink-0 overflow-hidden rounded border border-slate-200 bg-slate-100">
                                  {imageUrl ? (
                                    <img
                                      src={imageUrl}
                                ...
                                <div className="min-w-0 flex-1">
                                  <p className="truncate text-sm font-medium text-slate-900">{getItemTitle(item)}</p>
                                  <p className="mt-0.5 text-xs text-slate-500">
                                    SL: {Number(item?.quantity || 0)} • {formatCurrency(getItemLineTotal(item))}
                                  </p>
                                  <span className={`mt-1 inline-flex max-w-full rounded-full px-2 py-0.5 text-[10px] font-medium ${getStatusBadgeClass(itemStatus)}`}>
                    ...
                    <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-3">
                      ...
                      <div className="mt-3 space-y-3 text-sm">
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-slate-500">Trạng thái mặt hàng</span>
